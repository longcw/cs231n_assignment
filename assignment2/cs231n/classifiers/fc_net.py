...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        N = X.shape[0]
        mask = (np.arange(N), y)
        loss = -np.sum(np.log(probs[mask])) / N + 0.5 * self.reg * (np.sum(W1 * W1) + np.sum(W2 * W2))

        gt_scores = scores[mask].reshape((-1, 1))
        dsoft = -sum_scores / gt_scores / N
        tmp = np.zeros_like(scores)
        tmp[mask] = sum_scores.reshape(-1)
        tmp -= gt_scores
        dscore = dsoft * tmp / (sum_scores * sum_scores)
        dh2 = dscore * scores
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        b = self.params['b1']
        caches = dict()
        h, caches[1] = affine_relu_forward(X, W, b)

        for i in xrange(2, self.num_layers):
            W, b = self.params['W{}'.format(i)], self.params['b{}'.format(i)]
            h, caches[i] = affine_relu_forward(h, W, b)
        scores, caches[self.num_layers] = affine_relu_forward(h, self.params['W{}'.format(self.num_layers)], self.params['b{}'.format(self.num_layers)])
        ...
        N = X.shape[0]
        mask = (np.arange(N), y)
        loss = -np.sum(np.log(probs[mask])) / N
        ...
        for i in xrange(self.num_layers, 0, -1):
            dh, grads['W{}'.format(i)], grads['b{}'.format(i)] = affine_relu_backward(dh, caches[i])
